package com.example.projetoApiVendasEmSpring.customer.dto.corporateCustomerInput;
...
public record CorporateCustomerCreateDto(
        ...
        @NotBlank(message = "the cnpj must not be blank")
        @Size(min = 14, max = 14, message = "the cnpj must contain 14 characters")
        @Pattern(regexp = "\\d+",message = "the cnpj must contain only numbers")
        String cnpj
) {
}
